...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white flex flex-col items-center p-4 md:p-8 font-sans selection:bg-blue-500/30 relative overflow-hidden">
      ...
      <div className="w-full max-w-5xl flex flex-col items-center justify-center relative z-10 gap-12">

        {/* Header */}
        <div className="text-center space-y-4 max-w-2xl">
          <h1 className="text-3xl md:text-5xl font-black tracking-tight drop-shadow-lg leading-tight">
            Show the price of <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-blue-600">{tokenA ? tokenA.symbol.toUpperCase() : 'ETH'}</span> with the market cap of <span className="text-transparent bg-clip-text bg-gradient-to-r from-violet-400 to-violet-600">{tokenB ? tokenB.symbol.toUpperCase() : 'BTC'}</span>
          </h1>
        ...
        {/* Comparison Section */}
        <div className="flex flex-col md:flex-row items-center gap-6 w-full justify-center relative animate-in fade-in slide-in-from-bottom-5 duration-700 z-20">

          {/* Input A */}
          <div className="w-full md:w-[45%] z-20 flex flex-col gap-4">
            <TokenInput
              placeholder="Select Token (e.g. ETH)"
              selectedToken={tokenA}
              onSelect={(t) => setTokenAId(t.id)}
            />
          </div>

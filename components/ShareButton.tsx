...
export function ShareButton({ tokenA, tokenB, price, multiplier, className }: ShareButtonProps) {
    ...
    return (
        <button
            onClick={handleShare}
            className={clsx(
                `relative overflow-hidden group font-bold py-3 px-8 rounded-full transition-all flex items-center justify-center gap-2 shadow-lg hover:shadow-xl active:scale-95 z-20 w-full sm:w-auto ${className || ''}`,
                copied
        ...
        >
            ...
            {copied ? <Check className="w-5 h-5 relative z-10" /> : <Share2 className="w-5 h-5 relative z-10" />}
            <span className="relative z-10">{copied ? 'Copied!' : 'Share'}</span>
        </button>
    );
}

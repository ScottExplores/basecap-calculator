...
export function MarketCapDisplay({ tokenA, tokenB, amount = 1, onAmountChange, onSwapClick, userBalance }: MarketCapDisplayProps) {
    ...
    const priceB = tokenB.current_price || 0;
    const athPriceB = tokenB.ath || 0;
    const mcapB_Current = tokenB.market_cap || 0;

    const mcapB_ATH = priceB > 0 ? (mcapB_Current * (athPriceB / priceB)) : mcapB_Current;
    const targetMcapB = tokenB.isCustom ? mcapB_Current : (useATH ? mcapB_ATH : mcapB_Current);

    const multiplier = mcapA > 0 ? (targetMcapB / mcapA) : 0;
    ...
    const xValue = multiplier > 0 ? multiplier.toFixed(2) : "0.00";
        ...
        <div className="w-full bg-slate-800/50 backdrop-blur-md rounded-3xl p-8 border border-slate-700 shadow-2xl flex flex-col items-center relative overflow-hidden">
            ...
            <div className="mb-6 z-20 flex flex-col md:flex-row items-center justify-center gap-4 w-full max-w-2xl">
                ...
                {(tokenA?.symbol?.toUpperCase() === 'ETH' || tokenA?.symbol?.toUpperCase() === 'BTC' || (tokenA?.address && tokenA.address.startsWith('0x'))) ? (
                    ...
                    >
                        BUY {tokenA?.symbol?.toUpperCase() || 'TOKEN'}
                    </button>
                ) : (
            ...
            {!tokenB.isCustom && (
                <div className="flex bg-slate-900 rounded-lg p-1 mb-6 border border-slate-700 z-10">
                    ...
                    >
                        {tokenB?.symbol?.toUpperCase() || 'TARGET'} NOW
                    </button>
                    <button
                    ...
                    >
                        {tokenB?.symbol?.toUpperCase() || 'TARGET'} ATH
                    </button>
                </div>
            )}

            {/* Title */}
            <h3 className="text-xl md:text-2xl font-bold text-center mb-2 tracking-wide text-blue-200 uppercase">
                {tokenA?.symbol || 'TOKEN'} WITH THE MARKET CAP OF {tokenB?.isCustom ? formatMcap(targetMcapB) : (tokenB?.symbol || 'TARGET')}
            </h3>
            ...
            <div className="flex flex-col gap-6 w-full max-w-2xl mx-auto mt-8">
                ...
                <div className="space-y-2">
                    <div className="flex justify-between text-sm mobile-text-adjust">
                        <span className="font-bold text-slate-700 dark:text-slate-200">{tokenA?.symbol?.toUpperCase() || 'TOKEN'} NOW</span>
                ...
                <div className="space-y-2 opacity-50">
                    <div className="flex justify-between text-sm mobile-text-adjust">
                        <span className="font-bold text-slate-700 dark:text-slate-200">{tokenB?.symbol?.toUpperCase() || 'TARGET'} {tokenB?.isCustom ? '' : 'NOW'}</span>
